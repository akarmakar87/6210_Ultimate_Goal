...
public class StackDeterminationPipeline extends OpenCvPipeline {
    ...
    private volatile StackDeterminationPipeline.RingPosition position = StackDeterminationPipeline.RingPosition.FOUR;

    /**
     * PURPOSE: Class constructor to set starting position
     * @param lp
     */
    public StackDeterminationPipeline(boolean lp){
        leftPosition = lp;
    }
    ...
    public Mat processFrame(Mat input){
        ...
        position = StackDeterminationPipeline.RingPosition.FOUR; // Record our analysis
        if(avg1 > FOUR_RING_THRESHOLD){
            position = StackDeterminationPipeline.RingPosition.FOUR;
        }else if (avg1 > ONE_RING_THRESHOLD){
            position = StackDeterminationPipeline.RingPosition.ONE;
        }else{
            position = StackDeterminationPipeline.RingPosition.NONE;
        }
